...
////    }
//
//    // so we have to change this so that a simple bool will show that this exists
//    // but it really wastes a lot of space
//    // hmmmmmmmmmmmmm
//    // we can optionallt prove that it is an ethereum thing
//    // but as look as we store -- then we don't really case who it came from...
//
//
//    // so here we have proof of exitend...
//    // but we may also want to prove
//
//    mapping(bytes32 => bool) _exists;
//    mapping(bytes32 => bytes32) _existsWithProof;
//
//    // we waste
//
...
//        require(!_existsWithProof[hash], "Hashed with proof already exists");
...
//    /*
//        So, for example, if it were to be used for tokens. We can sign a legally binding
//        contract to provide something and that is mirrored in the real world exectuion
//sign(keccak256("\x19Ethereum Signed Message:\n" + len(message) + message)))
//
//    */
//    // actually this would be an object
////    function relaySignedProofOfExistence(bytes32 hash, bytes proof, string proofType) public {
////        require(isRegistered(_msgSender()), "Account not registered");
////        require(!_exists[hash], "Hash already recorded");
